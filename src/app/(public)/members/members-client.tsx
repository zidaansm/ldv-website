...
export function MembersClient({ initialMembers, initialTotalCount }: { initialMembers: Member[], initialTotalCount: number }) {
  ...
  useEffect(() => {
    // Skip fetching on initial load since we have initial data
    if (page === 1 && debouncedQuery === "" && members === initialMembers) return;
