...
export function StoryClient({ initialSections }: { initialSections: StorySection[] }) {
  const [sections, setSections] = useState<StorySection[]>(initialSections);
  const [loading, setLoading] = useState(false);
  const supabase = createClient();

  useEffect(() => {
    if (sections.length > 0 && sections === initialSections) return;
    ...
    fetchSections();
  }, [supabase, sections, initialSections]);
